...
object TopCountries extends RDDEventProcessingJob {
  ...
  private def calcTopCountriesByTotalRDD() = {
    events.cartesian(blocks).filter(pair => isInRange(pair._2(0), pair._1(4))).map(pair => (pair._2(1), pair._1(1)))
      .keyBy(_._1)
      .mapValues(_._2).mapValues(BigDecimal.exact)
      .reduceByKey(_ + _)
      .join(locations.keyBy(_ (0)).mapValues(_ (5)))
   ...
   def isInRange(network: String, ip: String) = {
    try {
